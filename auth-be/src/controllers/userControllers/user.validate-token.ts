...
const validateToken = asyncHandler(async(req: Request, res: Response) => {
    try {
        ...
        try {
            ...
            if(!userFromDB) {
                return res.status(404).json(new ApiError(404, "User not found in the database"));
            }
        ...
        return res.status(400).json(new ApiError(400, "There was an issue with processing the user request"));
    }
});
